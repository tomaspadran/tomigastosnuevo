...
const Dashboard = () => {
  ...
  const stats = useMemo(() => {
    ...
    return { total, chartData, filtered, events, aiSuggestions };
  ...
  const handleDelete = async () => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este gasto? Se eliminarán todas sus cuotas.')) {
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-200 p-4 pb-20">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          <TabsList className="bg-[#1e293b] border-slate-700">
            <TabsTrigger value="overview" className="text-xs uppercase font-bold text-white">Resumen Estadístico</TabsTrigger>
            <TabsTrigger value="calendar" className="text-xs uppercase font-bold text-white">Vista Calendario</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              <Card className="bg-[#1e293b] border-slate-700 border-l-4 border-l-blue-500">
                <CardHeader className="pb-2">
                  <CardTitle className="text-white text-[10px] font-bold uppercase tracking-widest">Total Periodo</CardTitle>
                </CardHeader>
                <CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card className="bg-[#1e293b] border-slate-700">
                <CardHeader><CardTitle className="text-sm font-bold uppercase text-white">Distribución por Categoría</CardTitle></CardHeader>
              ...
              <Card className="bg-[#1e293b] border-slate-700">
                <CardHeader><CardTitle className="text-sm font-bold uppercase text-white">Movimientos del Periodo</CardTitle></CardHeader>
                    ...
                    <div key={expense.id} className="flex items-center justify-between p-3 rounded-lg bg-[#0f172a] border border-slate-800">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-white/70 uppercase font-bold">{expense.type} • {expense.paid_by}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="font-black text-white text-sm">${Number(expense.amount).toLocaleString('es-AR')}</p>
                        <p className="text-[10px] text-white/50">{new Date(expense.date).toLocaleDateString('es-AR')}</p>
                      </div>
                    </div>
                  ))}
                </CardContent>
        ...
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          <DialogContent className="bg-[#1e293b] border-slate-700 text-white max-w-[95vw] sm:max-w-md rounded-2xl">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 uppercase font-black italic tracking-tighter text-white">
              ...
              <div className="py-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2 p-3 bg-[#0f172a] rounded-xl border border-slate-800">
                    <p className="text-[10px] text-white/50 uppercase font-bold mb-1">Descripción</p>
                  ...
                  <div className="p-3 bg-[#0f172a] rounded-xl border border-slate-800">
                    <p className="text-[10px] text-white/50 uppercase font-bold mb-1">Monto</p>
                    <p className="text-xl font-black text-emerald-400">${Number(selectedExpense.amount).toLocaleString('es-AR')}</p>
                  </div>
                  <div className="p-3 bg-[#0f172a] rounded-xl border border-slate-800">
                    <p className="text-[10px] text-white/50 uppercase font-bold mb-1">Pagado por</p>
            ...
            <DialogFooter className="flex flex-row gap-3">
              <Button 
                variant="destructive" 
                className="flex-1 bg-red-600 hover:bg-red-700 text-white font-black italic uppercase text-xs"
                onClick={handleDelete}
              >
                <Trash2 className="h-4 w-4 mr-2" /> Eliminar
              </Button>
              <Button 
                className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-black italic uppercase text-xs"
                onClick={() => {
                  setIsModalOpen(false);
      ...
      <style>{`
        ...
        .fc-col-header-cell { background: #0f172a; padding: 8px 0 !important; font-size: 0.65rem; color: #fff; font-weight: bold; }
